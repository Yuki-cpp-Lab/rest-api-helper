...
TEST_F(RestAPITest, TestDocsGeneration)
{
    auto& route = api_->add_route("test", "Test route");
    route.add_endpoint(
        HTTPMethod::HTTP_GET, [](const httplib::Request&, httplib::Response&) {}, "Get endpoint");
...
TEST_F(RestAPITest, TestDuplicateEndpointThrows)
{
    ...
    EXPECT_THROW(
        route.add_endpoint(
            HTTPMethod::HTTP_GET, [](const httplib::Request&, httplib::Response&) {}, "Duplicate"),
        std::runtime_error);
}
...
TEST_F(RestAPITest, TestPathParametersInDocs)
{
    ...
    route.add_endpoint(
        HTTPMethod::HTTP_GET, [](const httplib::Request&, httplib::Response&) {}, "Get user");
